		...
		<table border="0" cellpadding="10" cellspacing="0" width="600">
			<tr>
				<td colspan=4><input type="button" value="글수정" class="btn"
					...
					onclick="document.location.href='/myweb/mvc2bbs/list.do?pageNum=${pageNum}'">
				</td>
			</tr>
		</table>
	</form>
	...
	$(document).ready(function() {
		...
		$("#btn_update").click(function() {
			...
			if (session == 'guest' || session == null) {
				...
				location.href = "/myweb/mvc2bbs/updateForm.do?num=${article.num}&pageNum=${pageNum}&passwd=${mempw}&writer=${memid}";
				return;
			}
		});
		...
		$("#btn_del").click(function() {
			if (session == 'guest' || session == null) {
				...
				location.href = "/myweb/mvc2bbs/deletePro.do?num=${article.num}&pageNum=${pageNum}&passwd=${mempw}&writer=${memid}";
				return;
			}
		});
		...
		$("#btn_re").click(function() {
			if (session == 'guest' || session == null) {
				...
				location.href = "/myweb/mvc2bbs/writeForm.do?num=${article.num}&ref=${article.ref}&re_step=${article.re_step}&re_level=${article.re_level}";
				return;
			}
		});
	});
