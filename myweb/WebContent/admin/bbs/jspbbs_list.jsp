...
<c:if test="${count==0}">
	<table width="700">
		...
		<tr>
			<td><a href="writeForm.do"><img src="../../images/bt_write.gif" border="0"/></a></td>
		</tr>
	</table>
</c:if>	
<c:if test="${count>0}">
			...
			<tr>
				...
				<td width='260' align='left' class='tabletd'>
					<c:if test="${article.re_level>0}">
						<img src="../../mvc2bbs/images/level.gif" border="0" width="${5*article.re_level}" height="16" />
						<img src="../../mvc2bbs/images/re.gif">
					</c:if>
					<c:if test="${article.re_level==0}">
						<img src="../../mvc2bbs/images/level.gif" border="0" width="${5*article.re_level}" height="16" />
					</c:if>
					<a href="/myweb/admin/bbs/content.do?num=${article.num}&pageNum=${currentPage}">${article.subject}</a>
					<c:if test="${today == date}">
    					<img src="../../images/new.gif"/>
    				</c:if>
					<c:if test="${article.readcount>=10}">
						<img src="../../mvc2bbs/images/hot.gif" border="0" height="16" />
					</c:if>
				</td>
...
<c:if test="${count>0}">
	...
	<c:if test="${startPage>2 }">
		<a href="/myweb/admin/bbs/list.do?pageNum=${startPage2-1 }">[이전]</a>
	</c:if>
	<c:forEach var="i" begin="${startPage}" end="${endPage}" >
		<a href="/myweb/admin/bbs/list.do?pageNum=${i}">[${i}]</a>
	</c:forEach>
	<c:if test="${endPage<pageCount}">
		
		<a href="/myweb/admin/bbs/list.do?pageNum=${endPage2}">[다음]</a>
	</c:if>
</c:if>
<br/>
<!-- 
...
<script>
	$(document).ready(function(){
		...
		$("#write_bbs").click(function(){
				location.href="/myweb/admin/bbs/writeForm.do"
