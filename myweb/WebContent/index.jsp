...
String id = (String)session.getAttribute("s_id2");
if(id == null) {
	id = "guest";
}
%>
	...
	<div id="container">
    	...
    	<%
		    	...
		    	for(int idx=0; idx<noticeList.size(); idx++)
				{
					...
					String noticeContent = noticeDto.getSubject();
					if(noticeContent.length() > 18) {
						noticeContent = noticeContent.substring(0, 17) + "...";
					}

			 		out.print("<a href='./notice/noticeRead.jsp?nowPage="+nowPage+"&noticeno="+noticeDto.getNoticeno()+"'>"+noticeContent+"</a>");
		...
		<%
				for(int idx=0; idx<bbsList.size(); idx++)
				{
					...
					String bbsContent = bbsDto.getSubject();
					if(bbsContent.length() > 13) {
						bbsContent = bbsContent.subSequence(0, 12) + "...";
					}
			 		out.print("<a href='./bbs/bbsRead.jsp?nowPage="+nowPage+"&bbsno="+bbsDto.getBbsno()+"'>"+bbsContent+"</a>");
    	...
    	<%
		    	...
		    	for(int idx=0; idx<jspDTO.size(); idx++)
				{
					...
					//리플아이콘 출력
					if(mvcbbsDto.getRe_step() > 0) {
						for(int n=1; n<=mvcbbsDto.getRe_step(); n++) {
							out.print("<img src='./images/icon_reply.gif'/>");
						}
					}
					
					String mvcbbsContent = mvcbbsDto.getSubject();
    	...
    	<%
		    	...
		    	for(int idx=0; idx<phpDTO.size(); idx++)
				{
					...
					String phpbbsContent = phpbbsDto.getSubject();
	...
	<div id="footer">
		...
		<tr>
			<td class="footer">
				...
				<br/>
				 All Rights Reserved.   webmaster@devcomu.com
			</td>
		</tr>
		</table>	
	</div>
